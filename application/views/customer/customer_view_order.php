...
<div class="row">
          ...
          <div class="col-md-8">
            <h4>Order Details #<?php echo $orders['order_id'].' [ '.date('d/m/Y',strtotime($orders['created_date'])) ?> ]</h4>
						<?php
						...
						if($f>0)
						{
							?><a class="btn btn-warning" data-toggle="modal" href="#cancel_modal"> Cancel Order </a><?php
						}
						//NEW CODE
						if($orders['payment_type']=='Online Banking' && $orders['payment_date']=='')
						{
								?><a class="btn btn-primary" href="https://toyyibpay.com/<?php echo $orders['bill_id'] ?>"> Pay Now </a><?php
						}
						//CLOSE NEW CODE
						?>
            <hr>
											...
											via <b>
												...
												foreach ($order_status as $key) {
													foreach ($track as $key2) {
														if($key['seller_id']==$key2['seller_id'])
														{
															echo '<br>'.$key2['courier_name'].' ( Tracking No '.$key['shop_name'].' : '.$key2['tracking_code'].' )';
															if($key['order_status']!=4)
															{
																echo anchor('orders/receive_by_cust/'.$key2['order_id'].'/'.$key['order_status_id'].'/'.$key2['seller_id'], ' <button type="button" class="btn btn-warning" title="Click this if you received the order">Order Received</button>');
															}
									...
									<div class="form-group">
										...
										<div class="col-md-8">
											...
											<datalist id="bank-list">
												<?php
													foreach ($bank as $k) {
														?><option value="<?php echo $k['reference_note'] ?>"><?php echo $k['reference_note'] ?></option><?php
													}
												?>
											</datalist>
										</div>
