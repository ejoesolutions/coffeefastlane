  ...
  public function store_order_fpx()
  {
    ...
    if(isset($_POST['btnPlaceOrder'])){
      ...
      if($payment_type=="Online Banking"){
        ...
        $some_data = array(
          ...
          'billName'=> 'COFEE',
          'billDescription'=> 'COFEE',
          'billPriceSetting'=>1,
          'billPayorInfo'=>1,
          'billAmount'=>$total_all*100,
        ...
        $curl = curl_init();
          ...
          // redirect('https://toyyibpay.com/'.$billcode,'refresh');
          // $this->db->where('order_id',$order_id);
          // $this->db->update('ci_payment',array('bill_id'=>$billcode));
          $payment = array(
            'order_id' => $order_id,
            'payment_type' =>$_SESSION['payments'],
            'recorded_date'=>date("Y-m-d H:i:s"),
    ...
    <?php 
      ...
      if($_GET['status_id'] == 1){
        ...
        for ($f=0; $f < count($new_sellerID) ; $f++) {
          $this->data['seller']=$this->user_model->get_detail_seller($new_sellerID[$f]);
          $this->data['order_status'] = $this->order_model->get_order_status($_GET['order_id'],$new_sellerID[$f]);
          for($s=0;$s< count($_SESSION['sub_sellerID']);$s++){
              if($_SESSION['sub_sellerID'][$s]==$new_sellerID[$f]){
                $order_status=array(
                  'order_status'=>1,
